...
//           key - the key value
//           dataRid - the record id of the record associated with key
// Purpose : get the last pair (lastKey, lastPid) in the leaf page
//           and it's rid.
...
//
// Input   : None
// Output  : rid - record id of the half entry
//           key - the key value
//           dataRid - the record id of the record associated with key
// Purpose : get the pair (lastKey, lastPid) on the half the leaf page
//           and it's rid.
// Return  : OK if a record is returned,  DONE if no record exists on
//           this page.
//-------------------------------------------------------------------

Status BTLeafPage::GetHalf (int& key, RecordID& dataRid, RecordID& rid)
{
	rid.pageNo = pid;
	rid.slotNo = numOfSlots/2;
